...
# This uses a Linear Conflict Heuristic sort of heuristics, starting with the bottom row, then the second row and
#finally the first row placing tiles by priority of same line and goal.
...
class Puzzle(object):
    ...
    def moves(self):
        ...
        # YOU FILL THIS IN
        goalStateOne = [self.grid[0][0], self.grid[0][1], self.grid[0][2],self.grid[1][0], self.grid[1][1], self.grid[1][2], 6, 7, 8]
        goal = Puzzle([[' ', 1, 2], [3, 4, 5], [6, 7, 8]])
        for i in range(0 , len(goalStateOne)):
            if ( goalStateOne[i] == ' '):
                goalStateOne[i] = 0
        ...
        if (currentStateArray == goalStateOne):
            check = 1
        else:
            check = 0
                                ...
                                if (nodes[pop] == node):
                                    ...
                                    if (check != 1):
                                        ...
                                        if(node == 6  or node == 7 or node == 8):
                                            silence = "Silence is golden"
                                        elif (currentState[i][j] == node and goal.grid[i][j] == node):
                                            silence = "Silence is golden"
                                        else:
                                            curposMoves.append(node)
                            
            return list(set(curposMoves))
        ...
        if(currentStateArray != goalArray):
            ...
            print "Moves ", currentPossibleMoves, "Heuristics ", distanceHeuristics
            print self.h(currentStateArray)
    ...
    def h(self, goal):
        ...
        # YOU FILL THIS IN
        if(check == 0):
            goalArray = [self.grid[0][0], self.grid[0][1], self.grid[0][2],self.grid[1][0], self.grid[1][1], self.grid[1][2], 6, 7, 8]
            for i in range(0 , len(goalArray)):
                if (goalArray[i] == ' '):
                    goalArray[i] = 0
        if(check == 1):
...
def main():
    ...
    global puzzle, nodes, axes, currentStateArray, goalArray, goalStateOne, check
    check = 0
    nodes = []
